...
//else
	if(get_insn_type(re_ds.handle, arminst) == CS_GRP_RET){
...
*/
	...
	if(use->usetype == Opd && use->operand->type == ARM_OP_MEM)
		return;	
	...
	if(use->usetype == Opd)
		regid = use->operand->reg;		
	if(use->usetype == Base)
		regid = use->operand->mem.base;
	if(use->usetype == Index)
		regid = use->operand->mem.index;
